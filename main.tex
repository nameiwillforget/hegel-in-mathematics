...
\\
    ...
    Diese Repulsion, so als das Setzen der vielen Eins aber durch Eins selbst ist das eigne Außersichkommen
des Eins, aber zu solchen außer ihm, die selbst nur Eins sind. Es ist dieß die Repulsion dem Begriffe
nach, die an sich seyende. Die zweite Repulsion ist davon unterschieden, und ist die der Vorstellung der
äußern Reflexion zunächst vorschwebende, als nicht das Erzeugen der Eins, sondern nur als gegenseitiges
Abhalten vorausgesetzter, schon vorhandener Eins. Es ist dann zu sehen, wie jene an sich seyende Repulsion
...
=======
\textrm{\textesh} X$ factors through $\Im$: $X\rightarrow\Im X\rightarrow\textrm{\textesh} X$. Thus, $\Im
X$ can be understood as a contraction of $X$, where $X$ is not contracted down to its basic shape, but
only ``small'' paths in $X$ are contracted. More precisely, what is contracted are \emph{infinitesimal
paths}, and, correspondingly, the negative $\overline{\Im} X$ of $\Im$ on a connected space is the \emph{purely
infinitesimal} part of $X$. We conclude this stage of the process by positing an opposite to $\Im\dashv\&$,
given by a preceding moment $\Re$. These modalities $\Re\dashv\Im\dashv\&$ can together be taken to describe
the infinitesimal structure of $X$. Hegel described the infinitesimal thus in
>>>>>>> 58c76757763a3ec6e640a34c53bd94212b3fc9f8
...
First, given any moment $\bigtriangleup$, we can define a notion of \emph{$\bigtriangleup$-similarity}:
two types $S, T$ are $\bigtriangleup$-similar if $\bigtriangleup S = \bigtriangleup T$. For this we don't
need to assume the existence of $*$, but we do for the next notion: 

Personally, I am somewhat less convinced by this identification than the others. Hegel is talking about
the negative in the context of oppositions and seems to indicate that the negative is in some way the
inequality of moments, whereas this notion of negative can be constructed from any preceding moment. Clearly,
the categorical notion is of great mathematical importance, however, the connection to Hegel seems more
tenative. However, please note that negatives not play an essential role in the following derivation of
new unities, though they will be important for our understanding of some moments within the unities. For
instance, the negative of the moment $\varnothing$ is the \emph{maybe monad}, which adds a distinguished
point to every type $T$ and is of fundamental importance in computer science. \\



Questions:
Can an inverted Aufhebung exist?
Can an Aufhebung exist such that one component contains nothing of the initial unity? Does the existence
of the factorization $X \to \Im X \to \textrm{\textesh} X$ need  this assumption?
